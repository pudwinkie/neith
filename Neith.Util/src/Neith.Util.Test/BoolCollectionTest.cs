...
namespace Neith.Util.Test
{
    ...
    public class BoolCollectionTest
    {
        ...
        public void Test2()
        {
            ...
            IEnumerator<KeyValuePair<int, bool>> changeBits =
              BoolCollection.EnumBitChange(old, now).GetEnumerator();

            for (int i = 0; i < 8; i++) {
                Assert.IsTrue(changeBits.MoveNext());
                Assert.AreEqual(8 * i + i, changeBits.Current.Key);
                Assert.AreEqual(true, changeBits.Current.Value);
            }
        ...
        public void Test3()
        {
            ...
            for (int i = 0; i < 8; i++) {
                Assert.IsTrue(changeBits.MoveNext());
                Assert.AreEqual(8 * i + i, changeBits.Current.Key);
                Assert.AreEqual(false, changeBits.Current.Value);
            }
            Assert.IsFalse(changeBits.MoveNext());
        }
