...
namespace Neith.Util
{
    ...
    public static class CsvUtil
    {
        ...
        {
            Trace.TraceInformation("CSV FILE READ START [" + path + "]");
            using (TextFieldParser parser = new TextFieldParser(path, Encoding.GetEncoding(0))) {
            ...
            string path, int length, char commentChar, string endMark)
        {
            ...
            foreach (string[] rec in ReadCsv(path)) {
                ...
                if (rec[0][0] == commentChar) {
                    Trace.TraceInformation("コメント行を読み飛ばしました。File={0}:{1}", path, lineNo);
                    continue;
                }
                yield return rec;
            }
