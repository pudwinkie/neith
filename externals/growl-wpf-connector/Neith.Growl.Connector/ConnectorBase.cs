...
namespace Neith.Growl.Connector
{
    ...
    public abstract class ConnectorBase
    {
        ...
        /// <summary>
        /// The algorithm to use when generating hashes [defaults to SHA1]
        /// </summary>
        private Cryptography.HashAlgorithmType keyHashAlgorithm = Cryptography.HashAlgorithmType.SHA1;
        ...
        /// </summary>
        /// <value>
        /// <see cref="Cryptography.HashAlgorithmType"/> - Defaults to <see cref="Cryptography.HashAlgorithmType.SHA1"/>
        /// </value>
        ...
        /// <param name="state">An optional state object that will be passed into the response events associated with this request</param>
        protected abstract void OnCommunicationFailure(IResponse response, object state);
        ...
        protected void Send(MessageBuilder mb, ResponseReceivedEventHandler del, bool waitForCallback, object state)
        {
            ...
            var doSend = this.OnBeforeSend(mb);
            if (doSend)
            {
                var bytes = mb.GetBytes();
                mb = null;

                // start a new thread for the network connection stuff
                var cs = new ConnectionState(bytes, del, waitForCallback, state);
                var pts = new ParameterizedThreadStart(SendAsync);
        ...
        private void SendAsync(object obj)
        {
            ...
            {
                Neith.Growl.CoreLibrary.DebugInfo.WriteLine(ex.ToString());
            }
            finally
            {
                try
