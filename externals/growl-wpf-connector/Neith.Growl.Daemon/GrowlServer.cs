...
namespace Neith.Growl.Daemon
{
    ...
    public class GrowlServer : IDisposable
    {
        ...
        /// </summary>
        /// <param name="error">The <see cref="IError"/> that occurred</param>
        void mh_Error(IError error)
        {
        ...
        {
            var pts = new ParameterizedThreadStart(HandleParsedMessage);
        ...
        {
            var mh = (MessageHandler)obj;
            var request = mh.Request;
            ...
            try
            {
                ...
                switch (request.Directive)
                {
                    ...
                    case RequestType.NOTIFY:
                        var notification = Notification.FromHeaders(request.Headers);
        ...
        /// <param name="response">The <see cref="IResponse"/> to be written back</param>
        public void WriteResponse(CallbackInfo cbInfo, IResponse response)
        {
            if (!cbInfo.AlreadyResponded)
            {
                cbInfo.AlreadyResponded = true;
                var mh = cbInfo.MessageHandler;
                var request = mh.Request;
                var responseType = ResponseType.ERROR;
                if (response != null)
                {
                ...
                {
                    foreach (var item in cbInfo.AdditionalInfo)
                    {
                ...
                // return any application-specific data headers that were received
                var rd = RequestData.FromHeaders(request.Headers);
                AddRequestData(mb, rd);
        ...
        private void AddRequestData(MessageBuilder mb, RequestData requestData)
        {
            if (requestData != null)
            {
                var headers = requestData.ToHeaders();
