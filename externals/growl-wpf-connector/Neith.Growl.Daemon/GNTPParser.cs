...
namespace Neith.Growl.Daemon
{
    ...
    public class GNTPParser
    {
        ...
        /// </summary>
        /// <param name="request">The <see cref="IGNTPRequest"/> representing the parsed message</param>
        public delegate void GNTPParserMessageParsedEventHandler(IGNTPRequest request);
        ...
        public NextIndicator Parse(byte[] inputBytes)
        {
            try {
                ...
                if (tag == ACCEPT_TAG) {
                    ...
                    var pointer = this.pointers[this.currentPointer - 1];
                    pointer.ByteArray = bytes;
                    var binaryData = new BinaryData(pointer.Identifier, pointer.ByteArray);
        ...
        private void ParseEncryptedMessage(byte[] bytes)
        {
            ...
            var decryptedBytes = this.key.Decrypt(encryptedBytes, this.iv);
            ...
            using (var stream = new System.IO.MemoryStream(decryptedBytes))
            using (var reader = new GNTPStreamReader(stream)) {
                // main headers
