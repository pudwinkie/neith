...
namespace Neith.Growl.Daemon
{
    ...
    public class MessageHandler : Neith.Growl.Daemon.IMessageHandler
    {
        ...
        /// </summary>
        public delegate void MessageHandlerErrorEventHandler(IError error);
        ...
        private void SocketDidReadIndicatorBytes(AsyncSocket socket, byte[] readBytes, long tag)
        {
            ...
            if (tag == ACCEPT_TAG)
            {
                ...
                {
                    GNTPFlashSocketTraper gfsr = new GNTPFlashSocketTraper(socket, allowBrowserConnections);
                    gfsr.Read(readBytes);
                }
                else if (s == WebSocketHandshakeHandler.REQUEST_INDICATOR)
                {
                    ...
                    wshh.DoHandshake(delegate()
                    {
                        ...
                        GNTPWebSocketTraper gwsr = new GNTPWebSocketTraper(socket, passwordManager, passwordRequired, allowNetworkNotifications, allowBrowserConnections, allowSubscriptions, this.requestInfo);
                        this.requestReader = gwsr;
                        gwsr.MessageParsed += new GNTPRequestTraper.GNTPRequestReaderMessageParsedEventHandler(requestReader_MessageParsed);
                        gwsr.Error += new GNTPRequestTraper.GNTPRequestReaderErrorEventHandler(requestReader_Error);
                        gwsr.Read(readBytes);
                    });
                }
                else
                {
                    ...
                    GNTPSocketTraper gsr = new GNTPSocketTraper(socket, passwordManager, passwordRequired, allowNetworkNotifications, allowBrowserConnections, allowSubscriptions, this.requestInfo);
                    this.requestReader = gsr;
                    gsr.MessageParsed += new GNTPRequestTraper.GNTPRequestReaderMessageParsedEventHandler(requestReader_MessageParsed);
                    gsr.Error += new GNTPRequestTraper.GNTPRequestReaderErrorEventHandler(requestReader_Error);
                    gsr.Read(readBytes);
                }
            }
            else
        ...
        /// <summary>
        /// Handles the requestReader's <see cref="GNTPRequestTraper.MessageParsed"/> event
        /// </summary>
        /// <param name="request">The parsed <see cref="IGNTPRequest"/></param>
        void requestReader_MessageParsed(IGNTPRequest request)
        {
        ...
        /// <summary>
        /// Handles the requestReader's <see cref="GNTPRequestTraper.Error"/> event
        /// </summary>
        /// <param name="error">The <see cref="Error"/> information</param>
        void requestReader_Error(IError error)
        {
        ...
        private void WriteError(AsyncSocket socket, IError error)
        {
            if (this.Error != null)
            {
                this.Error(error);
            ...
            var headers = error.ToHeaders();
            var mb = new MessageBuilder(ResponseType.ERROR);
        ...
        private bool CheckAlreadyProcessed()
        {
            ...
            {
                foreach (var header in this.requestInfo.PreviousReceivedHeaders)
                {
