...
BIN_URL="http://binaries.mindtouch.com/trunk/"
REQUIRED_MONO="2.4"
DOWNLOADBIN=0
RECOMPILE=0
VERBOSE=0
...
then
	...
	if  [[ "$MONO_VERSION" < "$REQUIRED_MONO" ]]
	then
		echo "ERROR: installed Mono $MONO_VERSION is too old for running the current MindTouch release ( requires mono $REQUIRED_MONO or above )" >>$MyTTY
		echo "ERROR: installed Mono $MONO_VERSION is too old for running the current MindTouch release ( requires mono $REQUIRED_MONO or above )" 
		show_verbose_output ; exit 1
	fi
else
...
then
	wget -q $BIN_URL/_FILELIST.TXT -O /tmp/.filelist.tmp >>/dev/null 2>&1
	if [ $? -ne 0 ]; then
		echo "ERROR: Failure while fetching file list" >>$MyTTY
		echo "ERROR: Failure while fetching file list" 
		show_verbose_output ; exit
	fi

	echo "-- Downloading MindTouch binaries from $BIN_URL" >>$MyTTY
	cd $BASE/bin/
	DOWNLOADFAILED=0
	for FILE in `cat /tmp/.filelist.tmp`
	do 
		echo -e "$FILE \c " >>$MyTTY
		wget -q $BIN_URL/$FILE -O $FILE
		if [ $? -eq 0 ]
		then 
			echo "OK" >>$MyTTY
		else
