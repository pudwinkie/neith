...
namespace MindTouch.Deki.Data {
    public interface IDekiDataSession : IDisposable {
        ...
        uint Pages_GetCount();
        void Pages_GetDescendants(PageBE rootPage, string language, bool filterOutRedirects, out IList<PageBE> pages, out Dictionary<ulong, IList<ulong>> childrenInfo);
        ...
        RatingComputedBE Rating_GetResourceRating(ulong resourceId, ResourceBE.Type resourceType);
        RatingBE Rating_GetUserResourceRating(uint userId, ulong resourceId, ResourceBE.Type resourceType);
        void Rating_ResetUserResourceRating(uint userId, ulong resourceId, ResourceBE.Type resourceType, DateTime resetTimestamp);
        #endregion
        ...
        IList<ResourceBE> Resources_GetByQuery(IList<uint> parentIds, ResourceBE.Type parentType, IList<ResourceBE.Type> resourceTypes, IList<string> names, DeletionFilter deletionStateFilter, bool? populateRevisions, uint? offset, uint? limit);
        Dictionary<Title, AttachmentBE> Resources_GetFileResourcesByTitlesWithMangling(IList<Title> fileTitles);
        ...
        // TODO (brigettek):  This method returns partially populated PageBE's
        Dictionary<uint, IList<PageBE>> Tags_GetRelatedPages(IList<uint> tagids);
        ...
        IList<UserBE> Users_GetByIds(IList<uint> userIds);
        UserBE Users_GetByName(string userName);
        IList<UserBE> Users_GetByQuery(string userNameFilter, string realNameFilter, string userNameEmailFilter, string roleFilter, bool? activatedFilter, uint? groupId, uint? serviceIdFilter, SortDirection sortDir, UsersSortField sortField, uint? offset, uint? limit, out uint totalCount, out uint queryCount);
        uint Users_GetCount();
        uint Users_Insert(UserBE newUser);
        void Users_Update(UserBE user);
        IList<uint> Users_UpdateServicesToLocal(uint oldServiceId);
        #endregion     
