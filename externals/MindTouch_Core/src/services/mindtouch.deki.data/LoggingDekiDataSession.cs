...
namespace MindTouch.Deki.Data {
    public class LoggingDekiDataSession : IDekiDataSession, IDekiDataStats {
        ...
        //--- Constructors ---
        public LoggingDekiDataSession(IDekiDataSession nextSession) {
            if(nextSession == null) {
        ...
        private IDekiDataSession _next;

        //--- Properties ---
        public IDekiDataSession Head { get { return _next.Head; } set { } }
        ...
        public void Pages_GetDescendants(PageBE rootPage, string language, bool filterOutRedirects, out IList<PageBE> pages, out Dictionary<ulong, IList<ulong>> childrenInfo) {
            Stopwatch sw = Stopwatch.StartNew();
            _next.Pages_GetDescendants(rootPage, language, filterOutRedirects, out pages, out childrenInfo);
            LogQuery(CATEGORY_PAGE, "Pages_GetDescendants", sw, "rootPage", rootPage, "language", language, "filterOutRedirects", filterOutRedirects);
        }
        ...
        public RatingComputedBE Rating_GetResourceRating(ulong resourceId, ResourceBE.Type resourceType) {
            Stopwatch sw = Stopwatch.StartNew();
        ...
        public RatingBE Rating_GetUserResourceRating(uint userId, ulong resourceId, ResourceBE.Type resourceType) {
            Stopwatch sw = Stopwatch.StartNew();
        ...
        public void Rating_ResetUserResourceRating(uint userId, ulong resourceId, ResourceBE.Type resourceType, DateTime resetTimestamp) {
            Stopwatch sw = Stopwatch.StartNew();
        ...
        public Dictionary<uint, IList<PageBE>> Tags_GetRelatedPages(IList<uint> tagids) {
            Stopwatch sw = Stopwatch.StartNew();
            var ret = _next.Tags_GetRelatedPages(tagids);
        ...
        public IList<UserBE> Users_GetByIds(IList<uint> userIds) {
            Stopwatch sw = Stopwatch.StartNew();
        ...
        public IList<UserBE> Users_GetByQuery(string userNameFilter, string realNameFilter, string userNameEmailFilter, string roleFilter, bool? activatedFilter, uint? groupId, uint? serviceIdFilter, SortDirection sortDir, UsersSortField sortField, uint? offset, uint? limit, out uint totalCount, out uint queryCount) {
            Stopwatch sw = Stopwatch.StartNew();
            var ret = _next.Users_GetByQuery(userNameFilter, realNameFilter, userNameEmailFilter, roleFilter, activatedFilter, groupId, serviceIdFilter, sortDir, sortField, offset, limit, out totalCount, out queryCount);
            LogQuery(CATEGORY_USER, "Users_GetByQuery", sw, "userNameFilter", userNameFilter, "realNameFilter", realNameFilter, "userNameEmailFilter", userNameEmailFilter, "roleFilter", roleFilter, "activatedFilter", activatedFilter, "groupId", groupId, "serviceIdFilter", serviceIdFilter, "sortDir", sortDir, "sortField", sortField, "offset", offset, "limit", limit);
            return ret;
        }
        ...
        public IList<uint> Users_UpdateServicesToLocal(uint oldServiceId) {
            Stopwatch sw = Stopwatch.StartNew();
        ...
        public Dictionary<string, string> GetStats() {
            ...
            Dictionary<string, string> ret = (statSession == null) ? new Dictionary<string, string>() : new Dictionary<string, string>(statSession.GetStats());
            ret["log-writes"] = _loggedCallsCount.ToString();
            return ret;
        }

        private void LogQuery(string category, string function, Stopwatch sw, params object[] parameters) {
            if(sw != null) {
                sw.Stop();
            }
            if(IsLoggingEnabled) {
