...
";
        ...
        private void Rating_Compute(ulong resourceId, ResourceBE.Type resourceType, DateTime timestamp) {
...
")
        ...
        private RatingBE Rating_PopulateRating(IDataReader dr) {
            ...
            r.Id = dr.Read<uint>("rating_id");
            r.UserId = dr.Read<uint>("rating_user_id");
            r.ResourceId = dr.Read<uint>("rating_resource_id");
            r.ResourceType = (ResourceBE.Type)dr.Read<byte>("rating_resource_type");
            r.ResourceRevision = dr.Read<ulong?>("rating_resource_revision", null);
            r.Score = dr.Read<float>("rating_score");
            r.Timestamp = dr.Read<DateTime>("rating_timestamp");
            r.TimestampReset = dr.Read<DateTime?>("rating_reset_timestamp", null);
            return r;
        }

        private RatingComputedBE Rating_PopulateRatingComputed(IDataReader dr) {
            RatingComputedBE rc = new RatingComputedBE();
            rc.Id = dr.Read<uint>("ratingscomputed_id");
            rc.ResourceId = dr.Read<uint>("ratingscomputed_resource_id");
            rc.ResourceType = (ResourceBE.Type)dr.Read<byte>("ratingscomputed_resource_type");
            rc.Score = dr.Read<float>("ratingscomputed_score");
            rc.ScoreTrend = dr.Read<float>("ratingscomputed_score_trend");
            rc.Count = dr.Read<uint>("ratingscomputed_count");
            rc.Timestamp = dr.Read<DateTime>("ratingscomputed_timestamp");
            return rc;
        }
    }
}
