...
where
	gg.page_id = ?PAGEID;")
        ...
        public void Grants_DeleteByPage(IList<ulong> pageIds) {
            ...
            string pageIdsString = string.Join(",", DbUtils.ConvertArrayToDelimittedString(',', pageIds));
...
WHERE page_id in ({0});", pageIdsString);
        ...
        public void Grants_Delete(IList<uint> userGrantIds, IList<uint> groupGrantIds) {
            ...
            if (!ArrayUtil.IsNullOrEmpty(userGrantIds)) {
                userGrantIdsString = string.Join(",", DbUtils.ConvertArrayToDelimittedString(',', userGrantIds));
            }
            if (!ArrayUtil.IsNullOrEmpty(groupGrantIds)) {
                groupGrantIdsString = string.Join(",", DbUtils.ConvertArrayToDelimittedString(',', groupGrantIds));
            }
...
where page_id = ?TARGETPAGEID;")
        ...
        private Dictionary<T, PermissionStruct> Grants_CalculateEffectivePermissions<T>(IEnumerable<uint> userIds, IEnumerable<ulong> pageIds, bool keyByUser) {
            ...
            string pageIdsStr = string.Join(",", DbUtils.ConvertArrayToDelimittedString<ulong>(',', pageIds));
            string userIdsStr = string.Join(",", DbUtils.ConvertArrayToDelimittedString<uint>(',', userIds));
