    ...
    public partial class MySqlDekiDataSession {
        ...
        public IList<NavBE> Nav_GetTree(PageBE page, bool includeAllChildren) {
            ...
            if(page.ID == homepageId) {
                ...
                List<string> pageIdsTextList = DbUtils.ConvertArrayToStringArray(Head.Pages_GetParentIds(page).Where(x => (x != homepageId) && (x != 0)).Union(new[] { page.ID }));
                string parentIdsList = string.Join(",", pageIdsTextList.ToArray());
