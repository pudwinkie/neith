...
where file_id in ({0})
";
            Catalog.NewQuery(string.Format(query, string.Join(",", DbUtils.ConvertArrayToDelimittedString(',', fileIds))))
...
";
            Catalog.NewQuery(string.Format(query, string.Join(",", DbUtils.ConvertArrayToDelimittedString(',', resourceIds))))
            .Execute(dr => ret.AddRange(ResourceMapping_Populate(dr)));
            return ret;
        }
