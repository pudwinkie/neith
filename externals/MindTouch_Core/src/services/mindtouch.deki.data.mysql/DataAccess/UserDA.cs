...
namespace MindTouch.Deki.Data.MySql {
    public partial class MySqlDekiDataSession {
        ...
        public IList<UserBE> Users_GetByIds(IList<uint> userIds) {
            ...
            if(ArrayUtil.IsNullOrEmpty(userIds)) {
                return users;
            }
            string userIdsText = string.Join(",", DbUtils.ConvertArrayToStringArray<uint>(userIds).ToArray());
            Catalog.NewQuery(
...
select count(*) as querycount from users {0} {1};",
                                                    joinQuery, whereQuery, sortLimitQuery, groupId == null ? string.Empty : "where users.user_id = user_groups.user_id");
...
insert into users
	(user_role_id,user_name,user_password,user_newpassword,user_touched,user_email,user_service_id,user_active,user_real_name,user_external_name,user_create_timestamp,user_language,user_timezone)
values
	(?ROLEID, ?USERNAME, ?USERPASSWORD, ?NEWPASSWORD, ?TOUCHED, ?EMAIL, ?SERVICEID, ?ACTIVE, ?REALNAME, ?EXTERNALNAME, ?USER_CREATE_TIMESTAMP, ?USER_LANGUAGE, ?USER_TIMEZONE);
