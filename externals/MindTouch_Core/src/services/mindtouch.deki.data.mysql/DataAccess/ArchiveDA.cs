...
namespace MindTouch.Deki.Data.MySql {
    public partial class MySqlDekiDataSession {
        public void Archive_MovePagesTo(IList<ulong> pageIdsToArchive, uint transactionId) {
            ...
            string pageIdsStr = string.Join(",", DbUtils.ConvertArrayToDelimittedString<ulong>(',', pageIdsToArchive));
	...
	delete `old` o from `old` o, `pages` p
	where	o.old_page_id = p.page_id
	and 	p.page_id in ({0});
	...
	where page_id in ({0});", pageIdsStr))
        ...
        public void Archive_Delete(IList<uint> archiveIds) {
            ...
            string archiveIdsString = string.Join(",", DbUtils.ConvertArrayToDelimittedString(',', archiveIds));
