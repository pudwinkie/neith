...
{0}", userGroupQuery.ToString()))
        ...
        public void GroupMembers_UpdateUsersInGroup(uint groupid, IList<uint> userIds, DateTime timestamp) {
            string userIdsText = string.Join(",", DbUtils.ConvertArrayToDelimittedString<uint>(',', userIds));
            string deleteQuery = "delete from user_groups where group_id = ?GROUPID";
            StringBuilder insertQuery = new StringBuilder();
            if (userIds.Count > 0) {
                deleteQuery = string.Format("{0} and user_id not in ({1})", deleteQuery, userIdsText);
...
{0};
{1};", deleteQuery, insertQuery.ToString()))
        ...
        public IList<GroupBE> Groups_GetByIds(IList<uint> groupIds) {
            ...
            string groupIdsText = string.Join(",", DbUtils.ConvertArrayToDelimittedString<uint>(',', groupIds));
            return Groups_GetInternal(string.Format("where groups.group_id in ({0})", groupIdsText), "Groups_GetByIds");
        }
