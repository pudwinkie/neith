...
namespace MindTouch.Deki.Data.MySql {
    public partial class MySqlDekiDataSession {
      ...
      @"page_id,
       ...
       page_minor_edit,
       page_timestamp,
       char_length(page_text) AS page_text_length,
       page_revision,
       page_user_id,
       page_comment,
       page_usecache,
       ...
       page_etag";
        ...
        public IList<PageBE> Pages_GetByIds(IEnumerable<ulong> pageIds) {
            if(pageIds.Any()) {
                string pageIdsText = string.Join(",", DbUtils.ConvertArrayToDelimittedString(',', pageIds));
                return Pages_GetInternal(string.Format("WHERE page_id in ({0})", pageIdsText), null, null, "Pages_GetByIds");
            }
        ...
        public IList<PageBE> Pages_GetByTitles(IList<Title> pageTitles) {
            ...
            for(int i = 0; i < pageTitles.Count; i++) {
                ...
                sb.AppendFormat(" (page_namespace={0} AND page_title='{1}')", (uint)pageTitles[i].Namespace, DataCommand.MakeSqlSafe(pageTitles[i].AsUnprefixedDbPath()));
            }

            return Pages_GetInternal(string.Format("WHERE {0}", sb.ToString()), null, null, "Pages_GetByTitles");
        }
        ...
        public IList<PageBE> Pages_GetPopular(string language, uint? offset, uint? limit) {
            ...
            string query = "WHERE page_namespace = 0 AND page_is_redirect = 0";

            //Add language filter to query
            if(!string.IsNullOrEmpty(language)) {
                query += string.Format(" AND (page_language = '' OR page_language = '{0}') ", DataCommand.MakeSqlSafe(language));
            }

            // retrieve the list of popular pages
            return Pages_GetInternal(query, "ORDER BY page_counter DESC", limitAndOffset, "Pages_GetPopular");
        }
        ...
        public IList<PageBE> Pages_GetByNamespaces(IList<NS> namespaces, uint? offset, uint? limit) {
            ...
            if(namespaces != null && namespaces.Count > 0) {
                query = string.Format("WHERE page_namespace in ({0})", string.Join(",", namespaces.Select(x => ((uint)x).ToString()).ToArray()));
            }
            return Pages_GetInternal(query, "ORDER BY page_id", limitAndOffset, "Pages_GetByNamespaces");
        }

        public void Pages_GetDescendants(PageBE rootPage, string language, bool filterOutRedirects, out IList<PageBE> pages, out Dictionary<ulong, IList<ulong>> childrenInfo) {
...
@"JOIN watchlist wl
    ON wl.wl_user = {0} 
	AND wl.wl_title = page_title
	AND wl.wl_namespace = page_namespace"
, userid);
...
INSERT INTO pages ({0}page_namespace, page_title, page_text, page_comment, page_user_id, page_timestamp, page_counter, page_is_redirect, page_minor_edit, page_is_new, page_touched, page_usecache, page_toc, page_tip, page_parent, page_restriction_id, page_content_type, page_language, page_display_name, page_etag, page_revision)
VALUES ({1}?NAMESPACE, ?TITLE, ?PAGETEXT, ?PAGECOMMENT, ?USERID, ?PAGETIMESTAMP, ?COUNTER, ?ISREDIRECT, ?MINOREDIT, ?ISNEW, ?TOUCHED, ?USECACHE, '', ?TIP, ?PARENT, ?RESTRICTIONID, ?CONTENTTYPE, ?LANGUAGE, ?DISPLAYNAME, ?ETAG, ?REVISION);
{2}";
            query = restoredPageID != 0
                ? string.Format(query, "page_id,", "?PAGE_ID,", "SELECT " + restoredPageID + " as page_id")
                : string.Format(query, "", "", "SELECT LAST_INSERT_ID() as page_id;");
...
", updatePageText);
        ...
        public uint Pages_UpdateViewCount(ulong pageId) {
            return Catalog.NewQuery(@" /* Pages_UpdateViewCount */
UPDATE pages SET page_counter = (@counter := page_counter + 1) WHERE page_id = ?PAGEID;
SELECT CAST(@counter AS UNSIGNED);")
            .With("PAGEID", pageId)
            .ReadAsUInt() ?? 0;
        }
...
where p0.page_id = ?PAGEID;";
            ...
            throw new Exception(string.Format("GetParentPages failed: too many iterations. Starting id: {0} Parent chain: {1}", page.ID, string.Join(",", DbUtils.ConvertArrayToDelimittedString<ulong>(',', result))));
        }
        ...
        public IList<PageTextContainer> Pages_GetContents(IList<ulong> pageids) {
            ...
            string pageIdsText = string.Join(",", DbUtils.ConvertArrayToDelimittedString<ulong>(',', pageids));
            Catalog.NewQuery(
string.Format(
@"/* Pages_GetContents */
...
pageIdsText))
        ...
        private PageBE Pages_PopulatePage(IDataReader dr) {
            ...
            p.Counter = dr.Read<uint>("page_counter");
