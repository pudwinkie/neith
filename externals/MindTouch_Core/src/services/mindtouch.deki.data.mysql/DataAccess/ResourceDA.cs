...
namespace MindTouch.Deki.Data.MySql {
    public partial class MySqlDekiDataSession {
        ...
        public enum VisibilityFilter : int {
            ANY = -1,
            VISIBLEONLY = 0,
            HIDDENONLY = 1
        }
        ...
        private class ResourceQuery {
            ...
            public ResourceQuery WithChangeSetIds(uint changeSetId) {
                return WithChangeSetIds(new uint[] { changeSetId });
            }
            ...
            public ResourceQuery WithResourceType(ResourceBE.Type resourceType) {
                return WithResourceType(new ResourceBE.Type[] { resourceType });                
            }
            ...
            public ResourceQuery WithRevision(int? revision) {
                State s = new State(_state);
                s.resourceBE.Revision = revision ?? ResourceBE.HEADREVISION;
                s.populateRevisions = (revision ?? ResourceBE.HEADREVISION) != ResourceBE.HEADREVISION;
                return new ResourceQuery(s);
            }

            public ResourceQuery WithParent(uint? parentId, ResourceBE.Type parentType) {
                if(parentId == null) {
                    return WithParent(new uint[] { }, parentType);
                } else {
                    return WithParent(new uint[] { parentId.Value }, parentType);
                }
            }

            public ResourceQuery WithParent(IList<uint> parentIds, ResourceBE.Type parentType) {
                State s = new State(_state);
            ...
            public ResourceQuery IncludeContent(bool populateContent) {
                State s = new State(_state);
                s.populateContent = populateContent;
                return new ResourceQuery(s);
            }
            ...
            public ResourceQuery IncludeChildResources(bool populateChildResources) {
                State s = new State(_state);
                s.populateChildResources = populateChildResources;
                return new ResourceQuery(s);
            }

            public ResourceQuery IncludeChildResourcesForRev(bool populateChildResourcesForRev) {
                State s = new State(_state);
                s.populateChildResourcesForRev = populateChildResourcesForRev;
                return new ResourceQuery(s);
            }
            ...
            public string ToString(string methodName) {
                ...
                if(!ArrayUtil.IsNullOrEmpty(_state.resourceIds)) {
                    string resourceIdsStr = string.Join(",", DbUtils.ConvertArrayToDelimittedString<uint>(',', _state.resourceIds));
                    querySb.AppendFormat("\n AND {0}.res_id in ({1})", RESOURCE_ALIAS, resourceIdsStr);
                }

                if(!ArrayUtil.IsNullOrEmpty(_state.changeSetId)) {
                    string transactionIdsStr = string.Join(",", DbUtils.ConvertArrayToDelimittedString<uint>(',', _state.changeSetId));
                ...
                if(!ArrayUtil.IsNullOrEmpty(_state.resourceTypes)) {
                    byte[] tempTypes = Array.ConvertAll<ResourceBE.Type, byte>(_state.resourceTypes.ToArray(), new Converter<ResourceBE.Type, byte>(delegate(ResourceBE.Type t) { return (byte) t; }));
                    string resourceTypesStr = string.Join(",", DbUtils.ConvertArrayToDelimittedString<byte>(',', tempTypes));
                ...
                if(_state.resourceBE.Revision != ResourceBE.HEADREVISION) {
                    if(_state.resourceBE.Revision > ResourceBE.HEADREVISION) {
                        querySb.AppendFormat("\n AND {0}.resrev_rev = {1}", RREVISION_ALIAS, _state.resourceBE.Revision);
                    } else if(_state.resourceBE.Revision < ResourceBE.HEADREVISION) {
                        querySb.AppendFormat("\n AND {0}.resrev_rev = {1}.res_headrev - {2}", RREVISION_ALIAS, RESOURCE_ALIAS, Math.Abs(_state.resourceBE.Revision));
                    }
                }

                if(_state.parentType != ResourceBE.Type.UNDEFINED || !ArrayUtil.IsNullOrEmpty(_state.parentIds)) {

                    string parentIdsStr = (ArrayUtil.IsNullOrEmpty(_state.parentIds) ? "NULL" : string.Join(",", DbUtils.ConvertArrayToDelimittedString<uint>(',', _state.parentIds)));

                    switch(_state.parentType) {
                    case ResourceBE.Type.PAGE:
                        querySb.AppendFormat("\n AND {0}.resrev_parent_page_id in ({1})", RESOURCE_ALIAS, parentIdsStr);
                        break;
                    case ResourceBE.Type.USER:
                        querySb.AppendFormat("\n AND {0}.resrev_parent_user_id in ({1})", RESOURCE_ALIAS, parentIdsStr);
                        break;
                    case ResourceBE.Type.SITE:
                        ...
                        break;
                    default:
                        querySb.AppendFormat("\n AND {0}.resrev_parent_id in ({1})", RESOURCE_ALIAS, parentIdsStr);
                        break;
                ...
                //Child resources
                if(_state.populateChildResources || _state.populateChildResourcesForRev) {
...
{0}resrev_rev,{0}resrev_user_id,{0}resrev_parent_id,{0}resrev_parent_page_id,{0}resrev_parent_page_id,{0}resrev_parent_user_id,{0}resrev_change_mask,{0}resrev_name,{0}resrev_change_description,{0}resrev_timestamp,{0}resrev_content_id,{0}resrev_deleted,{0}resrev_changeset_id,{0}resrev_size,{0}resrev_mimetype,{0}resrev_language,{0}resrev_is_hidden,{0}resrev_meta",
            ...
            private static string BuildNameFilterWhereClause(IList<string> names, string tableAlias) {
                ...
                */
                ...
                if(nameSpecificQuery.Length > 0 || nameSubstringQuery.Length > 0) {
                    ret = string.Format(" AND ({0} {1} {2})", nameSpecificQuery.ToString(), (nameSpecificQuery.Length > 0 && nameSubstringQuery.Length > 0) ? "OR" : string.Empty, nameSubstringQuery.ToString());
                }
                return ret;
            }
        ...
        public Dictionary<Title, AttachmentBE> Resources_GetFileResourcesByTitlesWithMangling(IList<Title> fileTitles) {
            //A specialized query that retrieves file resources from titles. This is only used from DekiXmlParser.ConvertFileLinks
            
            Dictionary<Title, AttachmentBE> ret = new Dictionary<Title, AttachmentBE>();
    ...
    ";
            ...
            Catalog.NewQuery(query.ToString()).Execute(delegate(IDataReader dr) {
                while(dr.Read()) {
                    ...
                    AttachmentBE r = Resources_Populate(dr) as AttachmentBE;
                    if(r != null) {
                        ret[title] = r;
                    }
                }
...
", selectQuery);
        ...
        public void Resources_Delete(IList<uint> resourceIds) {
            ...
            string resourceIdsText = string.Join(",", DbUtils.ConvertArrayToDelimittedString<uint>(',', resourceIds));
...
delete from resourcerevs where resrev_res_id in ({0});", resourceIdsText))
        ...
        public void Resources_DeleteRevision(uint resourceId, int revision) {
            ...
            if(revision == AttachmentBE.TAILREVISION && rev.ResourceHeadRevision == revision) {
...
fr.resrev_res_id = f.res_id;")
        ...
        private static ResourceBE Resources_Populate(IDataReader dr) {
            ResourceBE.Type resourceType = (ResourceBE.Type)DbUtils.Convert.To<byte>(dr["res_type"], (byte)ResourceBE.Type.UNDEFINED);
            ResourceBE res = ResourceBE.New(resourceType);
            ...
            for(int i = 0; i < dr.FieldCount; i++) {
                ...
                case "res_headrev":
                    ...
                    break;
                case "res_type":
                    res.ResourceType = (ResourceBE.Type) (DbUtils.Convert.To<byte>(dr.GetValue(i)) ?? (byte) ResourceBE.Type.UNDEFINED);
                    break;
                case "res_deleted":
