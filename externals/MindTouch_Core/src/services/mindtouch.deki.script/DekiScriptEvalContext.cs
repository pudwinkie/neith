...
namespace MindTouch.Deki.Script {
    public class DekiScriptEvalContext {
        ...
        //--- Constants ---
        public const int MAX_NODE_COUNT = 100000;
        ...
        private readonly int MaxNodeCount = MAX_NODE_COUNT;
            ...
            if(!resultHead.IsEmpty) {
                foreach(XmlNode node in resultHead.AsXmlNode.ChildNodes) {
                    ...
                    if((node is XmlComment) ||
                        ((node is XmlElement) && (
                            StringUtil.EqualsInvariant(node.Name, "script") ||
                            StringUtil.EqualsInvariant(node.Name, "link") ||
                            StringUtil.EqualsInvariant(node.Name, "meta") ||
                            StringUtil.EqualsInvariant(node.Name, "style"))
                        )
                    ) {
        ...
        internal XmlCDataSection CreateCDataSection(string text) {
            if(System.Threading.Interlocked.Increment(ref _nodeCount) > MaxNodeCount) {
        ...
        internal XmlComment CreateComment(string text) {
            if(System.Threading.Interlocked.Increment(ref _nodeCount) > MaxNodeCount) {
                throw new DekiScriptDocumentTooLargeException(MaxNodeCount);
            }
