...
namespace MindTouch.Deki.Script.Runtime.Library {
    public static partial class DekiScriptLibrary {
        ...
        [DekiScriptFunction("string.match", "Match a string against a regular expression pattern and return the first capture.", IsIdempotent = true)]
        ...
        [DekiScriptFunction("string.matches", "Match a string against a regular expression pattern and return all captured parts.", IsIdempotent = true)]
        public static ArrayList StringMatches(
            ...
            if(m.Success && (m.Groups.Count > 1)) {
                ...
                // if we only have one capture group, put all captures into the result list
                if(m.Groups.Count == 2) {
                    for(int i = 0; i < m.Groups[1].Captures.Count; ++i) {
                        result.Add(m.Groups[1].Captures[i].Value);
                    }
                } else {

                    // create a result list per capture group
                    for(int j = 1; j < m.Groups.Count; ++j) {
                        ArrayList inner = new ArrayList();
                        for(int i = 0; i < m.Groups[j].Captures.Count; ++i) {
                            inner.Add(m.Groups[j].Captures[i].Value);
                        }
                        result.Add(inner);
                    }
                }
        ...
        public static Hashtable StringSplitCsv(
            ...
            // NOTE (steveb): CSV parser format based on http://www.creativyst.com/Doc/Articles/CSV/CSV01.htm
            ...
            for(int i = 0; i < rows.Length; ++i) {
                ...
                // loop over each character for the current row
                for(int j = 0; j < row.Length; ++j) {
                    ...
                    case '"':
                        if(quoted) {
                            ...
                            // check if we found two double-quotes in a row, if so append one and skip the next character
                    ...
                    case '\t':
                        if(quoted) {
                            ...
                            // append character since we're inside a quoted string
                            acc.Append(c);
                        } else {

                            // end of data value, appned to list of values for the current row
                            innerValues.Add(acc.ToString());
                            acc = new StringBuilder();
                        }
                        break;
                    default:

                        // append character only if we're inside a quoted string, the character is not a whitespace, of we have already accumulated a non-whitespace character
                        if(quoted || !char.IsWhiteSpace(c) || (acc.Length > 0)) {
                            acc.Append(c);
                        }
                        break;
                    }
                ...
                // check if we have an uncommitted value in the bufer
                if(acc.Length > 0) {
                    innerValues.Add(acc.ToString());
                }

                // check if the row contained any data
                if(innerValues.Count > 0) {
                    max = Math.Max(max, innerValues.Count);
                    values.Add(innerValues);
                }
            }

            // ensure that each row has max values
            for(int i = 0; i < values.Count; ++i) {
                ArrayList row = (ArrayList)values[i];
                while(row.Count < max) {
            ...
            result.Add("rowcount", values.Count);
            result.Add("columncount", max);
