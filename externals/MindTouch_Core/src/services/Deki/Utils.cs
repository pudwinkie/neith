...
namespace MindTouch.Deki {
    public static class Utils {
        ...
        internal const int MAX_DIFF_SIZE = 250000;
        ...
        internal static XDoc GetPageDiff(XDoc before, XDoc after, bool compact, out XDoc invisibleDiff, out string summary, out XDoc beforeHighlightedChanges, out XDoc afterHighlightedChanges) {
            XDoc result = after;
            ...
            Tuplet<ArrayDiffKind, XDocDiff.Token>[] diff = XDocDiff.Diff(before, after, MAX_DIFF_SIZE);
            if(diff == null) {
            ...
            if((added > 0) || (removed > 0) || (attributes > 0)) {
                try {
                    ...
                    if(invisibleChanges.Count > 0) {
                        ...
                        foreach(Tuplet<string, string, string> change in invisibleChanges) {
                            ...
                            if(change.Item2 != null) {
                                ...
                                invisibleDiff.Value(DekiResources.PAGE_DIFF_NOTHING);
                            }
                    ...
                    summary = string.Format(DekiResources.PAGE_DIFF_ERROR, "MindTouch", e);
                }
            }
            return result;
        }
        ...
        internal static string GetPageDiffSummary(PageBE page, string before, string beforeMime, string after, string afterMime) {
            ...
            Tuplet<ArrayDiffKind, XDocDiff.Token>[] diff = XDocDiff.Diff(beforeDoc.MainBody, afterDoc.MainBody, MAX_DIFF_SIZE);
            if(diff == null) {
        ...
        internal static void GetOffsetAndCountFromRequest(DreamContext context, uint defaultCount, out uint count, out uint offset, out SortDirection sortDirection, out string sortField) {
            ...
            if(countStr.EqualsInvariantIgnoreCase("all")) {
                ...
                throw new DreamBadRequestException(DekiResources.LIMIT_PARAM_INVALID);
            }
        ...
        internal static bool ShowPrivateUserInfo(UserBE u) {
            ...
            return (context.Instance.PrivacyExposeUserEmail) || (context.User.ID == u.ID) || (context.Deki.DetermineAccess(DreamContext.Current, DreamContext.Current.Request) == DreamAccess.Internal);
        }
        ...
        internal static Title GetRelToTitleFromUrl(DreamContext context) {
            ...
            if(0 == rootId) {
                ...
                if((null == rootPage) || (0 == rootPage.ID)) {
                    throw new DreamBadRequestException(DekiResources.INVALID_PAGE_ID);
                } else {
                    relToTitle = rootPage.Title;
                }
            }
            if((null != relToTitle) && relToTitle.IsTalk) {
                throw new DreamBadRequestException(DekiResources.CANNOT_RELTO_TALK);
            }
            return relToTitle;
        }
        ...
        private static string GetTranslatedName(this Title title) {
            ...
            case NS.USER:
                if(title.IsRoot) {
                    result = DekiResources.GetString("Page.ListUsers.page-title", null);
                }
                break;
            case NS.TEMPLATE:
                if(title.IsRoot) {
                    result = DekiResources.GetString("Page.ListTemplates.page-title", null);
                }
                break;
            case NS.ADMIN:
                if(title.IsRoot) {
                    result = DekiResources.GetString("Admin.ControlPanel.page-title", null);
                } else {
                    result = DekiResources.GetString("Admin.ControlPanel." + title.Path + ".page-title", null);
                }
                break;
            case NS.SPECIAL:
                if(title.IsRoot) {
                    result = DekiResources.GetString("Page.SpecialPages.page-title", null);
                } else {
                    result = DekiResources.GetString("Page.SpecialPages." + title.Path + ".page-title", null);
                }
                break;
            }
        ...
        private static string GetChangeSummary(Tuplet<ArrayDiffKind, XDocDiff.Token>[] diff, out int added, out int removed, out int attributes, out int structural) {
            added = 0;
            removed = 0;
            attributes = 0;
            structural = 0;
            ...
            string result;
            if((added > 0) && (removed > 0)) {
                result = string.Format(DekiResources.PAGE_DIFF_SUMMARY, added, removed);
            } else if(added > 0) {
                result = string.Format(DekiResources.PAGE_DIFF_SUMMARY_ADDED, added);
            } else if(removed > 0) {
                result = string.Format(DekiResources.PAGE_DIFF_SUMMARY_REMOVED, removed);
