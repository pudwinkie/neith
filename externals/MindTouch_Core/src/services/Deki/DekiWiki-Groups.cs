...
namespace MindTouch.Deki {
    ...
    public partial class DekiWikiService {
        ...
        public Yield GetGroupUsers(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            uint totalCount;
            uint queryCount;
            IList<UserBE> usersInGroup = UserBL.GetUsersByQuery(context, group.Id, out totalCount, out queryCount);
        ...
        private GroupBE GetGroupFromUrl() {
            ...
            if (groupid.StartsWith("=")) {
                ...
                if (!uint.TryParse(groupid, out groupIdInt)) {
                    throw new DreamAbortException(DreamMessage.BadRequest(DekiResources.GROUPID_PARAM_INVALID));
                }
            ...
            if (g == null) {
                string msg = byName ? string.Format(DekiResources.GROUP_NOT_FOUND, groupid.Substring(1)) : string.Format(DekiResources.GROUP_ID_NOT_FOUND, groupid);
                throw new DreamAbortException(DreamMessage.NotFound(msg));
            }
            return g;
        }
        #endregion
