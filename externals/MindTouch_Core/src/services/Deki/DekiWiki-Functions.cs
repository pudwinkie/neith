...
namespace MindTouch.Deki {
    ...
    public partial class DekiWikiService {
        ...
        public XDoc WikiToc(
            ...
            path = Title.FromUIUri(page.Title, path).AsPrefixedDbPath();
            if(StringUtil.EqualsInvariantIgnoreCase(page.Title.AsPrefixedDbPath(), path)) {
            ...
            page = GetPage(path, Permissions.READ, true);
            ...
            foreach(XDoc link in toc[".//@href"]) {
                ...
                if(StringUtil.StartsWithInvariant(anchor, "#")) {
        ...
        public XDoc WikiSearch(
            ...
            XDoc search = Coroutine.Invoke(DekiContext.Current.Deki.Search, query, (uint)(max ?? 10), (uint)(offset ?? 0), sortBy, constraint, new Result<XDoc>()).Wait();
        ...
        public XDoc WikiDirectory(
            ...
            search.Sort(delegate(XDoc left, XDoc right) {
                ...
                if(string.IsNullOrEmpty(leftTitle) && string.IsNullOrEmpty(rightTitle)) {
                    return 0;
                } else if(string.IsNullOrEmpty(leftTitle)) {
                    return -1;
                } else if(string.IsNullOrEmpty(rightTitle)) {
                    return 1;
                }
                ...
                } else if(char.IsLetter(leftTitle[0])) {
                    return 1;
                } else {
                    return -1;
                }
        ...
        public XDoc WikiLocalize(
            ...
            string resourceValue = DekiResources.GetString(resourceName);
            if(null != resourceParameters) {
                resourceValue = String.Format(resourceValue, resourceParameters.ToArray());
            }
        ...
        public XDoc WikiLanugages(
            ...
            foreach(DictionaryEntry entry in languages) {
                ...
                languageLink.Item1 = ci.TextInfo.ToTitleCase(ci.NativeName);
                languageLink.Item2 = Utils.AsPublicUiUri(Title.FromUriPath((string)entry.Value));
                languageLinks.Add(languageLink);
            }
            languageLinks.Sort(delegate(Tuplet<string, string> left, Tuplet<string, string> right) { return string.Compare(left.Item1, right.Item1, StringComparison.InvariantCulture); });
        ...
        public string WikiAppendPath(
            ...
            ArrayList titleList = title as ArrayList;
            if(titleList == null) {
        ...
        public ArrayList WikiComments(
            ...
            if(path == null) {
                ...
                page = GetPage(path, Permissions.NONE, true);
            }
            uint totalComments;
        ...
        public XDoc WikiCreate(
            ...
            string onClick = string.Format("window.location='/{0}'", StringUtil.EscapeString(pageTitle.AsUiUriPath(true)));

            // determine label
            label = label ?? DekiResources.NEW_PAGE;
        ...
        public XDoc WikiEdit(
            ...
            if(path != null) {
                ...
                // set onClick handler
                onClick = string.Format("window.location='{0}'", StringUtil.EscapeString(Utils.AsPublicUiUri(title)));
            } else {
                if(string.IsNullOrEmpty(section)) {
                    onClick = "doLoadEditor();return false;";
                } else {
                    onClick = string.Format("$('#{0} + :header > div.editIcon > a:first').click()", Title.AnchorEncode(section));
                }
            }

            // determine label
            label = label ?? DekiResources.EDIT_PAGE;
        ...
        public ArrayList WikiGetSearch(
            ...
            ArrayList result = new ArrayList();
            foreach(XDoc document in search["document"]) {
                if(StringUtil.EqualsInvariant(document["type"].AsText, "wiki")) {
                    result.Add(PageProperty(document["id.page"].AsUInt ?? 0, true));
                } else if(StringUtil.EqualsInvariant(document["type"].AsText, "comment")) {
        ...
        public Hashtable WikiVersion() {
            ...
            version.Add("major", v.Major);
            version.Add("minor", v.Minor);
            version.Add("build", v.Build);
            version.Add("revision", v.Revision);
            version.Add("text", v.ToString());
            version.Add("date", AssemblyUtil.GetBuildDate(deki));
            SvnRevisionAttribute svnRevision = AssemblyUtil.GetAttribute<SvnRevisionAttribute>(deki);
            if(svnRevision != null) {
                version.Add("svnrevision", svnRevision.Revision);
            }
            SvnBranchAttribute svnBranch = AssemblyUtil.GetAttribute<SvnBranchAttribute>(deki);
            if(svnBranch != null) {
        ...
        public XDoc WikiApi(
            ...
            try {
                ...
                if(!StringUtil.EqualsInvariant(source.Scheme, "local")) {
        ...
        internal ArrayList SiteUsers() {
            uint count;
            IList<UserBE> users = DbUtils.CurrentSession.Users_GetByQuery(null, null, null, null, true, null, null, SortDirection.ASC, UsersSortField.USERNAME, null, null, out count, out count);
            ArrayList result = new ArrayList();
        ...
        internal Hashtable PageRating(uint pageid) {
            ...
            if(p != null) {
                ret = MakeRatingObject(DbUtils.CurrentSession.Rating_GetResourceRating(pageid, ResourceBE.Type.PAGE));
            }
            return ret;
        }
        ...
        internal ArrayList TaggedProperty(uint tagid) {
            ...
            if (!ArrayUtil.IsNullOrEmpty(pageIds)) {
                ICollection<PageBE> pages = PageBL.GetPagesByIdsPreserveOrder(pageIds);
                if (pages != null) {
                    foreach (PageBE page in PermissionsBL.FilterDisallowed(DekiContext.Current.User, pages, false, Permissions.BROWSE)) {
        ...
        internal Hashtable FilesProperty(uint pageid) {
            ...
            PageBE page = GetPage(pageid, Permissions.READ, false);
            if(page != null) {
                foreach(AttachmentBE file in AttachmentBL.Instance.GetResources(page, DeletionFilter.ACTIVEONLY)) {
        ...
        internal Hashtable FileProperty(ulong id, bool isResourceId) {
            ...
            AttachmentBE file = AttachmentBL.Instance.GetResource((uint)id);
        ...
        internal string FileDescriptionProperty(uint resourceId) {
            PropertyBE description = PropertyBL.Instance.GetResource(resourceId, ResourceBE.Type.FILE, AttachmentBL.PROP_DESC);
            ...
            if((description != null) && description.MimeType.Match(MimeType.ANY_TEXT)) {
                return description.Content.ToText();
            }
            return null;
        }
        ...
        internal Hashtable PagePropertiesProperty(uint pageid) {
            ...
            PageBE page = GetPage(pageid, Permissions.READ, false);
            if(page != null) {
                foreach(PropertyBE property in PropertyBL.Instance.GetResources(pageid, ResourceBE.Type.PAGE)) {
        ...
        internal Hashtable UserPropertiesProperty(uint userid) {
            Hashtable result = new Hashtable(StringComparer.OrdinalIgnoreCase);
            foreach(PropertyBE property in PropertyBL.Instance.GetResources(userid, ResourceBE.Type.USER)) {
                Hashtable value = MakePropertyObject(property);
                if((value != null)&& (value.Count > 0)) {
        ...
        internal Hashtable FilePropertiesProperty(uint resourceId) {
            Hashtable result = new Hashtable(StringComparer.OrdinalIgnoreCase);
            foreach(PropertyBE property in PropertyBL.Instance.GetResources(resourceId, ResourceBE.Type.FILE)) {
                Hashtable value = MakePropertyObject(property);
                if((value != null)&& (value.Count > 0)) {
        ...
        internal Hashtable PropertyProperty(uint resourceid, int revision) {
            PropertyBE property = PropertyBL.Instance.GetResource(resourceid, revision);
        ...
        internal ArrayList PropertyRevisionsProperty(uint resourceid) {
            ArrayList result = new ArrayList();
            PropertyBE headRevision = PropertyBL.Instance.GetResource(resourceid);
        ...
        //--- Methods ---
        public override DekiScriptRuntime CreateScriptRuntime() {
        ...
        internal Hashtable MakePageObject(PageBE page, bool skipPermissionCheck, bool followRedirects) {
            ...
            if((page != null) && (page.ID != 0) && (skipPermissionCheck || PermissionsBL.IsUserAllowed(DekiContext.Current.User, page, Permissions.READ))) {
                ...
                result.Add("viewcount", page.Counter);
                ...
                if(parentID == 0) {
                    parentID = ((page.ID == instance.HomePageId) ? 0 : (ulong)instance.HomePageId);
                }
                result.Add("parent", PropertyAt("$page", parentID, followRedirects));
                result.Add("parents", PropertyAt("$parents", page.ID, followRedirects));

                if(page.IsHidden) {
                    DateTime? hiddenTs = page.MetaXml[ResourceBE.META_REVHIDE_TS].AsDate;
                    if(hiddenTs != null) {
                        result.Add("date.hidden", DekiScriptLibrary.CultureDateTime(hiddenTs.Value));
                    }
                    result.Add("description.hidden", page.MetaXml[ResourceBE.META_REVHIDE_COMMENT].AsText ?? string.Empty);
                    result.Add("user.hiddenby", PropertyAt("$user", page.MetaXml[ResourceBE.META_REVHIDE_USERID].AsUInt ?? 0));
                }
            }
            return result;
        }
        ...
        internal Hashtable MakeTagObject(TagBE tag) {
            Hashtable result = new Hashtable(StringComparer.OrdinalIgnoreCase);
            result.Add("name", tag.PrefixedName);
            result.Add("prefix", tag.Prefix);
            result.Add("value", tag.Name);
            result.Add("type", tag.Type.ToString().ToLowerInvariant());

            // add related pages
            if(tag.RelatedPages != null) {
                ArrayList pages = new ArrayList();
                foreach(PageBE related in tag.RelatedPages) {
        ...
        internal Hashtable MakeCommentObject(CommentBE comment) {
            ...
            entry.Add("text", comment.Content);
            entry.Add("mime", comment.ContentMimeType);
            entry.Add("date", DekiScriptLibrary.CultureDateTime(comment.CreateDate));
            entry.Add("author", PropertyAt("$user", comment.PosterUserId));
            entry.Add("page", PropertyAt("$page", comment.PageId));
            entry.Add("uri", PropertyAt("$commenturi", comment.PageId, comment.Number));
            entry.Add("number", comment.Number);
            return entry;
        }
