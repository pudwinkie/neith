...
namespace MindTouch.Deki {
    ...
    public partial class DekiWikiService {
        ...
        public Yield GetArchiveFile(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            AttachmentBE removedFile = GetAttachment(request, Permissions.NONE, true, true, out parentPage);

            DreamMessage responseMsg;
            if (!removedFile.ResourceIsDeleted)
                responseMsg = DreamMessage.NotFound(DekiResources.FILE_NOT_DELETED);
            else {
                StreamInfo file = DekiContext.Current.Instance.Storage.GetFile(removedFile, SizeType.ORIGINAL, false);
                if(file == null) {
                    throw new DreamInternalErrorException(string.Format(DekiResources.COULD_NOT_RETRIEVE_FILE, removedFile.ResourceId, removedFile.Revision));
                }
                responseMsg = DreamMessage.Ok(file.Type, file.Length, file.Stream);
        ...
        public Yield GetArchiveFileInfo(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            if (!removedFile.ResourceIsDeleted)
                ...
                responseMsg = DreamMessage.Ok(AttachmentBL.Instance.GetFileXml(removedFile, true, "archive", null));
        ...
        public Yield DeleteArchiveFile(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            AttachmentBE removedFile = GetAttachment(request, Permissions.NONE, true, true, out parentPage);
            if (!removedFile.ResourceIsDeleted)
                response.Return(DreamMessage.NotFound(DekiResources.FILE_NOT_DELETED));
            else {
                AttachmentBL.Instance.WipeAttachments(new AttachmentBE[] { removedFile });
        ...
        public Yield PostArchiveFilesRestore(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            if (to != string.Empty) {
                destPage = PageBL.GetPageFromPathSegment(false, to);
            }
