        ...
        private RoleBE GetRoleFromUrl(bool mustExist) {
            ...
            if (roleid.StartsWith("=")) {
                ...
                if (!uint.TryParse(roleid, out roleIdInt)) {
                    throw new DreamAbortException(DreamMessage.BadRequest(DekiResources.ROLEID_PARAM_INVALID));
                }
                r = PermissionsBL.GetRoleById(roleIdInt);
            }

            if (r == null && mustExist) {
                string msg = byName ? string.Format(DekiResources.ROLE_NAME_NOT_FOUND, roleid.Substring(1)) : string.Format(DekiResources.ROLE_ID_NOT_FOUND, roleid);
