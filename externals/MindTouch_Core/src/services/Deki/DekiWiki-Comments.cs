    ...
    public partial class DekiWikiService {
        ...
        public Yield GetPageComments(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            case "nondeleted":
                ...
                break;
            default:
                throw new DreamBadRequestException(DekiResources.FILTER_PARAM_INVALID);
            }
        ...
        public Yield PutCommentContent(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            if(comment != null) {
                DekiContext.Current.Instance.EventSink.CommentUpdate(context.StartTime, comment, page, DekiContext.Current.User);
                response.Return(DreamMessage.Ok(CommentBL.GetCommentXml(comment, null)));
            } else {
                response.Return(DreamMessage.InternalError(DekiResources.FAILED_EDIT_COMMENT));
            }
            yield break;
        }
        ...
        public Yield PostPageComment(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            if(comment != null) {
                DekiContext.Current.Instance.EventSink.CommentCreate(context.StartTime, comment, page, DekiContext.Current.User);
                response.Return(DreamMessage.Ok(CommentBL.GetCommentXml(comment, null)));
            } else {
                response.Return(DreamMessage.InternalError(DekiResources.FAILED_POST_COMMENT));
            }
            yield break;
        }
