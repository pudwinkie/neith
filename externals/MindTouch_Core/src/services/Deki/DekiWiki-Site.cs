...
namespace MindTouch.Deki {
    ...
    public partial class DekiWikiService {
        ...
        public Yield GetLocalizedString(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            if(value == null) {
                throw new DreamNotFoundException(string.Format(DekiResources.ERROR_NO_SUCH_RESOURCE, resource));
            }
        ...
        internal Yield GetSiteSettings(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            bool returnHiddenSettings = false;

            string givenKey = context.GetParam("apikey", null);
            ...
            if(!string.IsNullOrEmpty(givenKey)) {
                if(!string.IsNullOrEmpty(MasterApiKey)) {
                    ...
                    //Master api key is set. Request key matches master key.
                    if(MasterApiKey == givenKey) {
                        returnHiddenSettings = true;
                    }
                }

                //Master api key not set. Request key matches instance key.
                else if(givenKey == ConfigBL.GetInstanceSettingsValue("security/api-key", null)) {
                    returnHiddenSettings = true;
                }
            }
            XDoc doc = ConfigBL.GetInstanceSettingsAsDoc(returnHiddenSettings);
        ...
        internal Yield PutSiteSettings(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            if(!MimeType.XML.Match(request.ContentType)) {
                throw new DreamBadRequestException(DekiResources.EXPECTED_XML_CONTENT_TYPE);
            }
            var dekiContext = DekiContext.Current;
            XDoc settings = request.ToDocument();
            ConfigBL.SetInstanceSettings(settings);
            dekiContext.Instance.EventSink.InstanceSettingsChanged(context.StartTime);
