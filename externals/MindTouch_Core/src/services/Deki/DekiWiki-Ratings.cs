    ...
    public partial class DekiWikiService {
        ...
        public Yield PostPageRating(DreamContext context, DreamMessage request, Result<DreamMessage> response) {
            ...
            if(UserBL.IsAnonymous(user)) {
                throw new DreamAbortException(DreamMessage.AccessDenied(AUTHREALM, string.Format(DekiResources.OPERATION_DENIED_FOR_ANONYMOUS, string.Empty)));
            }
            PageBE page = PageBL.AuthorizePage(user, Permissions.READ, false);
            ...
            if(!string.IsNullOrEmpty(scoreStr)) {
                ...
                if(!float.TryParse(scoreStr, out tempScore)) {
                    throw new DreamBadRequestException(DekiResources.RATING_INVALID_SCORE);
                }
                score = tempScore;                
            }
