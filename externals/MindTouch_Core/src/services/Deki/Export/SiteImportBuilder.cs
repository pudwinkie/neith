                    ...
                    if(importXml.HasName("page")) {
                        ...
                        if(!_forceoverwrite) {
                            ...
                            if(page != null && page.ID != 0 && !page.IsRedirect) {
                                ...
                                var lastImport = PropertyBL.Instance.GetResource((uint)page.ID, ResourceBE.Type.PAGE, LAST_IMPORT);
                                if(lastImport != null) {
                                    try {
                        ...
                        if(importXml["filename"].IsEmpty) {
                            throw new DreamBadRequestException(DekiResources.MISSING_FILENAME);
                        }
                        ...
                        if(importXml["name"].IsEmpty) {
                            throw new DreamBadRequestException(String.Format(DekiResources.UNDEFINED_NAME, "name"));
                        }
                        ...
                        if(importXml["filename"].IsEmpty) {
                            ...
                            if(!_uris.ContainsKey(uri)) {
                                throw new DreamBadRequestException(String.Format(DekiResources.FILENAME_IS_INVALID));
                            }
                        }
