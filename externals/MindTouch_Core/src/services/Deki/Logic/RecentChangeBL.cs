...
namespace MindTouch.Deki.Logic {
    ...
    public static class RecentChangeBL {
        ...
        public static void AddPageMetaRecentChange(DateTime timestamp, PageBE title, UserBE user, string comment) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, title, user, comment, 0, RC.PAGEMETA, 0, String.Empty, false, 0);
        }

        public static void AddTagsRecentChange(DateTime timestamp, PageBE title, UserBE user, string comment) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, title, user, comment, 0, RC.TAGS, 0, String.Empty, false, 0);
        }

        public static void AddGrantsAddedRecentChange(DateTime timestamp, PageBE title, UserBE user, string comment) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, title, user, comment, 0, RC.GRANTS_ADDED, 0, String.Empty, false, 0);
        }

        public static void AddGrantsRemovedRecentChange(DateTime timestamp, PageBE title, UserBE user, string comment) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, title, user, comment, 0, RC.GRANTS_REMOVED, 0, String.Empty, false, 0);
        }

        public static void AddRestrictionUpdatedChange(DateTime timestamp, PageBE title, UserBE user, string comment) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, title, user, comment, 0, RC.RESTRICTION_UPDATED, 0, String.Empty, false, 0);
        }

        public static void AddUserCreatedRecentChange(DateTime timestamp, PageBE title, UserBE user, string comment) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, title, user, comment, 0, RC.USER_CREATED, 0, String.Empty, false, 0);
        }

        //TODO (MaxM): Add rc_type for file restore, upload, delete, move, description, wipe
        public static void AddFileRecentChange(DateTime timestamp, PageBE title, UserBE user, string comment, uint transactionId) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, title, user, comment, 0, RC.FILE, 0, String.Empty, false, transactionId);
        }

        public static void AddNewPageRecentChange(DateTime timestamp, PageBE title, UserBE user, string comment) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, title, user, comment, 0, RC.NEW, 0, String.Empty, false, 0);
        }
        ...
        public static void AddDeletePageRecentChange(DateTime timestamp, PageBE page, UserBE user, string comment, bool minorChange, uint transactionId) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, page, user, comment, 0, RC.PAGEDELETED, 0, string.Empty, minorChange, transactionId);
        }

        public static void AddRestorePageRecentChange(DateTime timestamp, PageBE page, UserBE user, string comment, bool minorChange, uint transactionId) {
            DbUtils.CurrentSession.RecentChanges_Insert(timestamp, page, user, comment, 0, RC.PAGERESTORED, 0, string.Empty, minorChange, transactionId);
        }

        public static void AddCommentCreateRecentChange(DateTime timestamp, PageBE page, UserBE user, string summary, CommentBE comment) {
            AddCommentRecentChange(timestamp, page, user, summary, comment, RC.COMMENT_CREATE);
        }

        public static void AddCommentDeleteRecentChange(DateTime timestamp, PageBE page, UserBE user, string summary, CommentBE comment) {
