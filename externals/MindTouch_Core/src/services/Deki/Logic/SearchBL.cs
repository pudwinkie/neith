...
namespace MindTouch.Deki.Logic {
    ...
    public class SearchBL : ISearchBL {
        ...
        //--- Constructors ---
        public SearchBL(IDekiDataSession session, IKeyValueCache cache, string wikiid, XUri apiUri, Plug searchPlug, UserBE user, IInstanceSettings settings, SearchQueryParser parser, Func<bool> adaptiveSearchEnabled) {
            _session = session;
            _cache = cache;
            _wikiid = wikiid;
            _apiUri = apiUri;
        ...
        public SearchQuery BuildQuery(string queryString, string constraint, SearchQueryParserType parserType, bool haveApiKey) {
            ...
            case SearchQueryParserType.Filename:
                ...
                var fileQuery = string.Format("filename:{0}* extension:.{0}", term);
        ...
        public SearchResult GetCachedQuery(SearchQuery query) {
            ...
            if(_cache.TryGet(GetQueryKey(query.LuceneQuery), out result)) {
                _log.DebugFormat("got query for '{0}' from cache", query);
            } else {
                _log.DebugFormat("query '{0}' was not in cache", query);
            }
            return result;
        }
        ...
        private void ThrowOnInvalidLicense() {
            if(!IsAdaptiveSearchEnabled) {
                throw new MindTouchLicenseInvalidOperationException("Adaptive Search Log API");
            }
        }
        ...
        private void BuildLookupQueries(List<SearchResultItem> items, Dictionary<string, SearchResultItem> itemLookup, List<string> queries, string prefix, string postfix) {
            ...
            foreach(var item in items) {
                ...
                itemLookup.Add(item.Type + ":" + item.TypeId, item);
