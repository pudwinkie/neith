...
namespace MindTouch.Deki.Logic {
    public static class RatingBL {
        ...
        private static bool IsRatingsEnabled {
            get {
                ...
                return (licenseState == LicenseStateType.TRIAL || licenseState == LicenseStateType.COMMERCIAL) 
                    && ((LicenseBL.GetCapability(LicenseBL.CONTENTRATING) ?? string.Empty).EqualsInvariantIgnoreCase(LicenseBL.CONTENTRATING_ENABLED));
            }
        }
        ...
        public static XDoc AppendRatingXml(XDoc doc, PageBE page, UserBE user) {
            ...
            RatingComputedBE ratingEffective = DbUtils.CurrentSession.Rating_GetResourceRating(page.ID, ResourceBE.Type.PAGE);
            ...
            if(ratingEffective != null) {
                ...
                userRating = DbUtils.CurrentSession.Rating_GetUserResourceRating(user.ID, page.ID, ResourceBE.Type.PAGE);
                if(userRating != null) {
        ...
        private static void ThrowOnInvalidLicense() {
            if(!IsRatingsEnabled) {
                throw new MindTouchLicenseInvalidOperationException("Content Rating API");
            }
        }
    }
}