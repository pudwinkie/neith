...
class OpenIdCreateUser extends SpecialPagePlugin
{
	...
	public function displayForm() {
		...
		$html .= "<p>Your OpenID is not yet associated with a wiki user account.</p>";
		
		$html .= '<form id="createuser" method="post" class="createuser">';

		$html .= '<div class="field">';
		$html .= DekiForm::singleInput('text', 'username', null, array(), 'Username:');
	    $html .= '&nbsp;<span id="available"></span>';
		$html .= '</div>';
		
		$html .= "<br/>";
		
		$html .= DekiForm::singleInput('button', 'action', 'save', array(), "Create");
		$html .= DekiForm::singleInput('hidden', 'openid', urlencode($openid));
		$html .= DekiForm::singleInput('hidden', 'hash', md5($wgDekiApiKey . $openid));

		$html .= '</form>';
		$html .= "<script language=\"JavaScript\">$('#createuser').find (':submit').attr ('disabled', 'disabled');</script>";
