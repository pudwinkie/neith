...
class OpenIdLogin extends SpecialPagePlugin
{
	...
	protected function &output()
	{
		...
		if (!is_null($Request->getVal('openid.mode')))
		{
			...
			$Plug = new DekiPlug($wgDekiOpenIdService);
	...
	public function displaySelector()
	{
		...
		return '<script type="text/javascript"><!--//' . PHP_EOL . '$(function() { $(\'#openid\').openid({}); });  //--></script><form method="post" action="' . $Title->getLocalUrl() . '" id="openid"><input type="hidden" name="returnto" value="' . htmlspecialchars($Request->getVal('returntotitle')) . '"/></form>';
	}
}
endif;
