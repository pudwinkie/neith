...
namespace MindTouch.Deki.Tests.PageTests {
    ...
    public class CommentTests
    {
        ...
        [Ignore]
        [Test]
        public void Bug0003842_TestAnonymousCanPostComment()
        {
            // Bug http://bugs.opengarden.org/view.php?id=3842

            Plug p = Utils.BuildPlugForAnonymous();

            DreamMessage msg = p.At("users", "current").Get();
            Assert.AreEqual(DreamStatus.Ok, msg.Status);
            Assert.IsFalse(msg.ToDocument()["permissions.user/operations"].AsText.Contains("VIEW"));

            try
            {
                msg = DreamMessage.Ok(MimeType.TEXT, "This is anonymous comment text");
                msg = p.At("pages", "home", "comments").Post(msg);
                Assert.IsTrue(false);
            }
            ...
            try
            {
                msg = p.At("pages").Get();
                Assert.AreEqual(DreamStatus.Ok, msg.Status);
                if (!msg.ToDocument()["page/subpages/page[1]/@id"].IsEmpty)
                {
                    Assert.IsFalse(msg.ToDocument()["page/subpages/page[1]/@id"].IsEmpty);
                    string id = msg.ToDocument()["page/subpages/page[1]/@id"].AsText;

                    msg = DreamMessage.Ok(MimeType.TEXT, "This is anonymous comment text");
                    msg = p.At("pages", id, "comments").Post(msg);
                    Assert.IsTrue(false);
                }
            }
            catch (DreamResponseException ex)
