...
Deki.Dialog.prototype.render = function()
{
    ...
    this.registerPlugin(new Deki.Dialog.Wysiwyg(this));
    ...
    this.firePluginEvent("BeforeOpen");
}
...
Deki.Dialog.prototype.close = function(returnedParams)
{
    ...
    if ( YAHOO.lang.isFunction(callback) && YAHOO.lang.isValue(returnedParams) )
    {
        // Run the callback function and pass to it the returned param
        callback.call(scope, returnedParams);
    }
    
	this.firePluginEvent("BeforeClose");
...
Deki.Dialog.prototype.registerPlugin = function(plugin)
{
    this.plugins[plugin._pluginInfo.name] = plugin;
}

Deki.Dialog.prototype.firePluginEvent = function(methodName)
{
    for ( var i in this.plugins )
    {
        var plugin = this.plugins[i];
        if ( plugin && YAHOO.lang.isFunction(plugin[methodName]) )
        {
            if ( plugin[methodName].apply(plugin) )
            {
                return true;
            }
        }
