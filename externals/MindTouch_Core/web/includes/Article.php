...
class Article {
	...
	function getContent() {
		...
		if ($this->getId() == 0) 
		{
			...
			// if you're not logged-in, and the page doesn't exist, redirect to the homepage
			if ($wgUser->isAnonymous()) 
			{
				$wgOut->redirectHome();
				return;	
	...
	function addsubpage() {
	    global $wgOut, $wgRequest;
 	    ...
 	    $parent = $this->mTitle->getPrefixedText();
	    $_children = $this->mTitle->getArticleId() > 0 ? $this->getChildren(true): array();

	    $children = array();
	    foreach ($_children as $key => $val) {
		   $e = explode('|', $key);
	       $children[strtolower($e[3])] = $e[3];
       }
	    $index = 0;

	    while (true) {
		    $title = wfMsg('Article.Common.new-page-title', ($index > 1 ? ' '.$index : ''));
	        if (!array_key_exists(trim(strtolower($title)), $children)) {
	           break;
	        }
	        ++$index;
	    }

	    $newName = wfMsg('Article.Common.new-page-title', ($index > 0 ? " $index" : ''));
	    if ($this->mTitle->getDBkey() != '') {
            $newName = $this->mTitle->getText() != wfHomePageInternalTitle()
            	? $this->mTitle->getText() . HPS_SEPARATOR . $newName
            	: $newName;
    	}
		$nt = Title::makeTitle( $this->mTitle->getNamespace(), $newName );
		$location = $nt->getFullURL('action=edit', true);
		if ($nt->getArticleID() > 0) {
		    $location .= "&redirect=no&subpage=true";
		}
	...
	static function &getNextSubpageTitle(&$ParentTitle, $subpageName)
	{
	    ...
	    $allChildren = Article::getAllChildren(null, $ParentTitle);

	    $children = array();
	    foreach ($allChildren as $child)
		{
			$pt = Title::newFromText($child['path']);
			$titleText = $pt->getPathlessText();
			$children[strtolower($titleText)] = $child['title'];
		}
		...
		{
			$index++;
		    $titleText = $subpageName . ($index > 0 ? ' ' . $index : '');
	    } while (array_key_exists(strtolower($titleText), $children));

	    $newName = $subpageName . ($index > 0 ? ' ' . $index : '');

	    if ($ParentTitle->getDBkey() != '')
		{
            $newName = $ParentTitle->getText() != wfHomePageInternalTitle()
            	? $ParentTitle->getText() . HPS_SEPARATOR . $newName
            	: $newName;
    	}
		$nt = Title::makeTitle($ParentTitle->getNamespace(), $newName);
	...
	function view()	{
		...
		$this->loadContent(); //load the object from the API
