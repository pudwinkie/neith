...
class Skin {
    ...
    /***
     * MT royk
     * Outputs JS markup that is required for functionality of site
     */
	function showEditorJavascript() {
       global $wgArticle, $wgUser, $wgArticle, $wgRequest, $wgTitle, $wgContLang, $wgLanguageCode, $wgServer;
        if ($wgRequest->getVal('wpNewPath'))
            $titleFull = Article::combineName($wgRequest->getVal('wpNewPath'), $wgRequest->getVal('wpNewTitle'));
        else
            $titleFull = $wgTitle->getPrefixedText();
        ...
        if (Skin::isNewPage())
        {
	        ...
	        $js .= "Deki.CancelUrl = '" . str_replace("'", "\'", $cancel_url) . "';" . "\n";
        }
        elseif ($wgRequest->getVal('action') == 'source')
        {
        ...
        if ($wgRequest->getVal('action') == 'edit' || $wgRequest->getVal('action') == 'submit') {
        	...
        	$js .= 'Deki.StaticEditor = true;' . "\n";
        }
        
		global $wgGalleryRows, $wgGalleryCols, $wgLocalCssPath;

		if (empty($titleName)) {
		    $titleName = wfHomePageTitle();
		}
		
        $js .= "Deki.BaseHref = '" . $wgServer . "';" . "\n";

        $js .= 'Deki.PageId = ' . $title->getArticleID() . ';' . "\n";
        $js .= "Deki.PageTitle = '" . wfEncodeJSString($titleFull) . "';" . "\n";
        $js .= "Deki.PageLanguageCode = '" . wfEncodeJSString($wgLanguageCode) . "';" . "\n";
        $js .= "Deki.FollowRedirects = " . ($wgRequest->getVal('redirect') == 'no' ? '0': '1') . ";" . "\n";
		
		// path to current skin template (e.g. /skins/ace). Isn't used now.
		$js .= "Deki.PathTpl = '" . wfEncodeJSString(Skin::getTemplatePath()) . "';" . "\n";		
		// path to current skin (e.g. /skins/ace/blue). Isn't used now.
        $js .= "Deki.PathSkin = '" . wfEncodeJSString(Skin::getSkinPath()) . "';" . "\n";
        // path to common dir (/skins/common)
        $js .= "Deki.PathCommon = '" . wfEncodeJSString(Skin::getCommonPath()) . "';" . "\n";

        // used for autoreplase of ~~~ and ~~~~ in the editor
        $js .= "Deki.UserName = '" . htmlspecialchars(wfEncodeJSString($wgUser->getName())) . "';" . "\n";
        $js .= "Deki.Today = '" . $wgContLang->date(date('YmdHis'), false) . "';" . "\n";
        
        // editor area custom css
        $js .= 'Deki.EditorStyles = [];' . "\n";
        $js .= "Deki.EditorStyles.push('" . wfEncodeJSString(Skin::getTemplatePath()) . "/_editor.css');" . "\n";
        $js .= "Deki.EditorStyles.push('" . wfEncodeJSString(Skin::getSkinPath()) . "/_content.css');" . "\n";
        
        if (!empty($wgLocalCssPath))
        {
            $js .= "Deki.EditorStyles.push('" . wfEncodeJSString($wgLocalCssPath) . "');" . "\n";
        }
	...
	function getAnalytics() {
		$analytics = wfGetConfig('ui/analytics-key');
		if (empty($analytics)) {
			return;
		}
		...
		return '<script type="text/javascript">'
			...
			.($analytics == $wgDefaultAnalytics ? 'pageTracker._setDomainName(".mindtouch.com");': '')
