 ...
 * @see MT: #0004335: FCKeditor: ~~~ and ~~~~ should be replaced automaticly
 */
FCKEnterKey.prototype.DoEnter = function( mode, hasShift )
{
	// >MT: Autoreplace functionality
	if ( typeof( this._AutoReplace ) == 'function' )
		this._AutoReplace() ;
	// <MT
 ...
 * @see MT: #0004335: FCKeditor: ~~~ and ~~~~ should be replaced automaticly
 */
FCKEnterKey.prototype._AutoReplace = function()
{
	if ( typeof(FindReplace) == 'undefined' )
		return false ;
	
	var parentElement = FCKSelection.GetBoundaryParentElement( true ) ;
	var parentPath = new FCKElementPath( parentElement ) ;
	var block = parentPath.Block ;

	if ( block && ! ( block.nodeName.toLowerCase() == 'pre' ) )
	{
		var elUserLink = FCK.EditorDocument.createElement( 'a' ) ;
		elUserLink.className = 'internal' ;
		elUserLink.title = 'User:' + FCKConfig.UserName ;
		elUserLink.href = MindTouchDeki.InternalPrefix + 'User:' + FCKConfig.UserName ;
		elUserLink.innerHTML = FCKConfig.UserName ;
		
		var range = new FCKDomRange( this.Window ) ;
		range.MoveToSelection() ;

		var bookmark = range.CreateBookmark() ;

		var oFindReplace = new FindReplace() ;
		oFindReplace.ReplaceAll( block, '~~~~', [ elUserLink, FCK.EditorDocument.createTextNode( ' ' + FCKConfig.Today ) ], false, true, false ) ;

		var oFindReplace = new FindReplace() ;
		oFindReplace.ReplaceAll( block, '~~~', [ elUserLink ], false, true, false ) ;
...
/**
 * @see MT: #0004931: Hitting "backspace" should not delete a table
 */
FCKEnterKey.prototype._ExecuteBackspace = function( range, previous, currentBlock )
{
	...
	if ( previous && currentBlock )
	{
		...
		{
			// >MT: don't delete table by backspace
			if ( sPreviousName != 'table' )
			{
...
FCKEnterKey.prototype._ExecuteEnterBlock = function( blockTag, range )
{
	...
	if ( oSplitInfo )
	{
		...
		else
		{
			if ( bIsStartOfBlock && bIsEndOfBlock )
			{
				// >MT: definition lists support
				switch ( ePreviousBlock.tagName.toUpperCase() )
				{
					case 'LI':
					...
					case 'DT':
						...
						FCKDomTools.MoveChildren( ePreviousBlock.parentNode.removeChild( ePreviousBlock ), eBlock ) ;
						FCKDomTools.InsertAfterNode( ePreviousBlock.parentNode, eBlock ) ;
						return true ;
						// break ;
				}
				// <MT
			...
			if ( ePreviousBlock )
			{
				...
				// >MT: definition lists support
				if ( sPreviousBlockTag.Equals(['DT', 'DD']) )
				{
					eNewBlock = this.Window.document.createElement( sPreviousBlockTag == 'DT' ? 'DD' : 'DT' ) ;
				}
				// <MT
				// If is a header tag, or we are in a Shift+Enter (#77),
				// create a new block element (later in the code).
				else if ( !this._HasShift && !(/^H[1-6]$/).test( sPreviousBlockTag ) )
				{
			...
			{
				// >MT: definition lists support
				if ( eNextBlock.tagName.toUpperCase() == 'DD' )
				{
					eNewBlock = this.Window.document.createElement( 'DT' ) ;
				}
				// <MT
				else
...
FCKEnterKey.prototype._ExecuteEnterBr = function( blockTag )
{
	...
	if ( oRange.StartBlockLimit == oRange.EndBlockLimit )
	{
		...
		// >MT: see #0006243
		var bIsPre = sStartBlockTag.IEquals( 'pre' ) ;
//		var bIsPre = false ;
		// <MT
		...
		if ( ( !bHasShift && bIsEndOfBlock && (/^H[1-6]$/).test( sStartBlockTag ) )
			|| ( bHasShift && bIsPre && FCKConfig.EnterMode == 'br' ) ) // >MT: see #0006243
		{
			// >MT: see #0006243
			bIsPre = false ;
			// <MT
		...
		// >MT: see #0006243
		else if ( bHasShift && bIsPre )
		{
			// Exit from pre block if shift+enter pressed
			bIsPre = false ;
			this._ExecuteEnterBlock( FCKConfig.EnterMode, oRange ) ;
		}
		// >MT: fixed segfault in some cases in FF
		// see #0006241
		// let FF handle enter key itself in this case
		else if ( FCKBrowserInfo.IsGecko && bIsPre )
		{
