...
FCKStyle.prototype =
{
	 ...
	 * 
	 * Fix for appling of inline styles to selected cells of tables in FF
	 * @see #4741
	 */
	ApplyToSelection : function( targetWindow )
	{
		...
		{
			this._ToTable( range, this.ApplyToRange ) ;
		}
		else
		{
	...
	RemoveFromSelection : function( targetWindow )
	{
		...
		if ( MindTouchDeki.CheckTable() )
		{
			this._ToTable( range, this.RemoveFromRange ) ;
		}
		else
		{
	...
	RemoveFromRange : function( range, selectIt, updateRange )
	{
		...
		// >MT: if Element is block store the start block
		// @see #0006983
		var startBlock = null ;
		if ( this.GetType() != FCK_STYLE_INLINE )
		...
		// >MT: if Element is block get the parent block of the bookmark
		// @see #0006983
		if ( startBlock && startBlock.nodeName.toLowerCase() == this.Element )
		{
			var currentNode = startBlock ;
		}
		else
		{
		...
		while ( currentNode )
		{
			...
			if ( currentNode.nodeType == 1 )
			{
				...
				if ( mayRemove )
				{
					...
					// >MT: Remove only inline elements
					// @see #0006983
					if ( this.GetType() == FCK_STYLE_INLINE )
	...
	CheckElementRemovable : function( element, fullMatch )
	{
		...
		if ( elementName == this.Element )
		{
			...
			{
				// >MT: also skip _fckstyle
				if ( att == '_length' || att == '_fckstyle' )
					continue ;
				// <MT
	...
	{
		// >MT: check _fckstyle attribute
		var elementStyle = element.getAttribute( '_fckstyle' ) || element.style.cssText ;
		// <MT
		...
		if ( elementStyle.length == 0 || elementStyle == ';' )
		{
			...
			FCKDomTools.RemoveAttribute( element, '_fckstyle' ) ;
		}
		else
		{
			elementStyle = elementStyle.replace( regex, '' ) ;
			element.style.cssText = elementStyle ;
			element.setAttribute( '_fckstyle', elementStyle ) ;
		}
	},
	...
	BuildElement : function( targetDoc, element )
	{
		...
		{
			var styleValue = this.GetFinalStyleValue() ;
			el.style.cssText = styleValue ;
			el.setAttribute( '_fckstyle', styleValue ) ;
		}
	...
	{
		if ( ( attName == 'style' || attName == '_fckstyle' ) && valueA && valueB )
		{
	 ...
	 * @see #4741
	 */
	_ToTable : function( range, callback )
	{
		...
		for ( var i = 0 ; i < aCells.length ; i++ )
		{
			...
			var oClone = range.Clone() ;
			oClone.SetStart( eCell, 1 ) ;
			oClone.SetEnd( eCell, 2 ) ;
	
			callback.apply( this, [ oClone, false, true ] ) ;
		}
	},
	...
	_ApplyInlineStyle : function( range, selectIt, updateRange )
	{
		...
		while ( currentNode )
		{
			...
			// >MT: see #0006613
			if ( !nodeName || ( elementDTD[ nodeName ] && ( FCKDomTools.ComparePosition( currentNode, endNode ) | FCK_POSITION_FOLLOWING | FCK_POSITION_IDENTICAL | FCK_POSITION_CONTAINS ) == ( FCK_POSITION_FOLLOWING + FCK_POSITION_IDENTICAL + FCK_POSITION_CONTAINS ) ) )
			// <MT
			{
				...
				if ( ( FCK.DTD[ currentNode.parentNode.nodeName.toLowerCase() ] || FCK.DTD.span )[ elementName ] || !FCK.DTD[ elementName ] )
				{
					...
					// >MT: see #0006613
					if ( !range.CheckHasRange() && ( !nodeName || !FCKListsLib.RemoveEmpty[ nodeName ] || ( FCKDomTools.ComparePosition( currentNode, endNode ) | FCK_POSITION_FOLLOWING | FCK_POSITION_IDENTICAL | FCK_POSITION_CONTAINS ) == ( FCK_POSITION_FOLLOWING + FCK_POSITION_IDENTICAL + FCK_POSITION_CONTAINS ) ) )
	...
	_GetAttribsForComparison : function()
	{
		...
		if ( this._GetStyleText().length > 0 )
		{
			...
			attribs['_fckstyle'] = attribs['style'] ;
		}
