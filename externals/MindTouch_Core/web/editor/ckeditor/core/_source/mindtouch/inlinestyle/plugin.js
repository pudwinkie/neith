...
(function()
{
	...
	{
		return parseStyle( element.getAttribute( '_cke_saved_style' ) );
	}
	...
	function setSavedStyles( element, value )
	{
		if ( checkDocument( element ) )
			element.$.setAttribute( '_cke_saved_style', value );
	}
			...
			CKEDITOR.tools.extend( CKEDITOR.dom.element.prototype,
				{
					...
					getStyleAttribute : (function()
					{
						...
						{
							if ( !isOriginal && this.hasAttribute( '_cke_saved_style' ) )
							{
								return this.$.getAttribute( '_cke_saved_style', 2 );
							}
							else
						...
						if ( CKEDITOR.env.ie && ( CKEDITOR.env.ie7Compat || CKEDITOR.env.ie6Compat ) )
						{
							return function( isOriginal )
							{
								if ( !isOriginal && this.hasAttribute( '_cke_saved_style' ) )
								{
									return this.$.getAttribute( '_cke_saved_style' );
								}
								else
									// IE does not return inline styles via getAttribute(). See #2947.
									return this.$.style.cssText;
					...
					removeAttribute : (function()
					{
						var standard = function( name )
						{
							 ...
							 * @author MindTouch
							 */
							if ( name == 'style' && this.hasAttribute( '_cke_saved_style' ) )
								this.$.removeAttribute( '_cke_saved_style' );
							/* END */
						};
					...
					removeStyle : function( name )
					{
						...
						if ( !this.hasAttribute( '_cke_saved_style' ) )
						{
							this.setStyle( name, '' );
						...
						if ( !savedStyle )
							...
							this.$.setAttribute( '_cke_saved_style', savedStyle );
						/* END */
					},
					...
					copyAttributes : function( dest, skipAttributes )
					{
						 ...
						 * @author MindTouch
						 */
						if ( this.$.style.cssText.length || this.hasAttribute( '_cke_saved_style' ) )
							dest.$.style.cssText = this.getAttribute( '_cke_saved_style' ) || this.$.style.cssText;
						/* END */
					}
