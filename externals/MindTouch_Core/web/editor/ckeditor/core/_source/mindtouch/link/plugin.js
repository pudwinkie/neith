...
(function()
{
	...
	var linkCmd =
	{
		...
		exec : function( editor )
		{
			...
			var url =
				[
					...
					'&cntxt=' + editor.config.mindtouch.pageTitle.utf8URL(),
		...
		_ :
		{
			...
			insertLink : function( params )
			{
				...
				attributes._cke_saved_href = mindtouchLink.normalizeUri( params.f_href );				
				attributes.title = mindtouchLink.hrefDecode( attributes._cke_saved_href );
				...
				if ( !this._.selectedElement )
				{
					...
					{
						var anchor = ( params.f_text.length > 0 ) ? params.f_text : params.f_href;
						var text = new CKEDITOR.dom.text( anchor, editor.document );
	...
	var mindtouchLink =
	{
			...
			editor.ui.addButton( 'MindTouchLink',
				{
					...
					command : pluginName,
					icon : plugin.path + 'images/a_link.png'
				} );
			...
			editor.ui.addButton( 'Unlink',
				{
					...
					command : 'unlink',
					icon : plugin.path + 'images/a_unlink.png'
				} );
		...
		hrefDecode : function( href )
		{
			if( typeof href.replace == 'undefined' )
			{
				href = href.toString();
			}
			...
			href = unescape( href );
			href = href.utf8ToString();
			
			if ( href.indexOf( this.internalPrefix ) == 0 )
			{
				href = href.replace( /_/ig, " " );
			}
