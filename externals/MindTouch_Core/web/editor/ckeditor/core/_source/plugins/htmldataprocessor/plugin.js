...
(function()
{
	 ...
	 * @author MindTouch
	 */
	var protectStyleAttributeRegex = /<[^>\s]+?\s[^>]*?((?:style)\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|(?:[^ "'>]+)))/gi;
	/* END */
	...
	function protectAttributes( html )
	{
		 ...
		 * @author MindTouch
		 */
		html = html.replace( protectAttributeRegex, '$& _cke_saved_$1' );
		html = html.replace( protectStyleAttributeRegex, '$& _cke_saved_$1' );
		return html;
