...
class EmailSettings extends DekiControlPanel
{
	...
	protected function POST_form()
	{
		...
		if ($action == 'email') 
		{
			...
			$emailUi = DekiMailer::sendEmail(
				$User->getEmail(),
				$this->View->msg('EmailSettings.test.subject', $wgSitename),
				$this->View->msg('EmailSettings.test.body', $wgSitename),
				$this->View->msg('EmailSettings.test.body.html', $wgSitename)
			);
			
			$Result = DekiMailer::sendTestEmail(
				$User->getEmail(),
				$this->View->msg('EmailSettings.test.api.subject', $wgSitename),
				$this->View->msg('EmailSettings.test.body', $wgSitename),
				$this->View->msg('EmailSettings.test.body.html', $wgSitename)
			);
