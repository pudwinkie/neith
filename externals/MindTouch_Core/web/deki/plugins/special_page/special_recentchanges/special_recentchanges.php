...
class SpecialRecentChanges extends SpecialPagePlugin
{
	...
	{		
		...
		{
			$labels[] = wfMsg('Page.RecentChanges.ns.'. (empty($option) ? 'all' : $option));
		}
		$options = array_combine($options, $labels);
	...
	protected function getData()
	{
		...
		$this->namespace = $this->Request->getEnum('namespace', self::$namespaces, self::DEFAULT_NAMESPACE_FILTER);
		...
		{
			$Plug = $Plug->With('namespace', $this->namespace);
		}
