...
namespace GameOfLife
{
    ...
    internal class GameBoard
    {
        ...
        public GameBoard(int width, int height, double initialDensity, ObjectPool<Bitmap> pool)
        {
            ...
            _entityState = new Color?[2][,] { new Color?[width, height], new Color?[width, height] };
            ...
            for (int i = 0; i < width; i ++)
            {
                ...
                {
                    _entityState[_currentIndex][i, j] = (rand.NextDouble() < initialDensity) ? Color.FromArgb(rand.Next()) : (Color?)null;
                }
            }
        }
        ...
        public Bitmap MoveNext()
        {
            ...
            Color?[,] current = _entityState[_currentIndex];
            Color?[,] next = _entityState[nextIndex];
            Random rand = new Random();
