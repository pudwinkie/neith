...
public static class GameLogic
{
    ...
    public static Tuple<FrameworkElement,MediaElement> MakeVictoryScreen(int remaining)
    {
        ...
        var sound = new MediaElement() { LoadedBehavior = MediaState.Manual, Source = new System.Uri(remaining > 0 ? "booo.wav" : "happykids.wav", System.UriKind.Relative) };
        content.Children.Add(title);
    ...
    public static GameData InitializeGameData()
    {
        ...
        for (int y = 0; y < HEIGHT; y++)
        {
            for (int x = 0; x < WIDTH; x++)
            {
                ...
                System.Diagnostics.Debug.WriteLine("{0} - {1} - {2}", x, (float)x / (float)WIDTH, e.Fill);
